...
public class GLProgram {
    ...
    private int _video_width = 720;
    private int _video_height = 1280;
    // flow control
     ...
     * 测试直接画
     */
    public void test(Buffer bitmap, int width, int height) {
        boolean videoSizeChanged = (width != _video_width || height != _video_height);
        if (videoSizeChanged) {
            _video_width = width;
            _video_height = height;
        ...
        checkGlError("glBindTexture");
        GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_LUMINANCE, _video_width, _video_height, 0,
                GLES20.GL_LUMINANCE, GLES20.GL_UNSIGNED_BYTE, bitmap);
        checkGlError("glTexImage2D");
        GLES20.glTexParameterf(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_MIN_FILTER, GLES20.GL_NEAREST);
        GLES20.glTexParameterf(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_MAG_FILTER, GLES20.GL_LINEAR);
        GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_WRAP_S, GLES20.GL_CLAMP_TO_EDGE);
    ...
    public int createProgram(String vertexSource, String fragmentSource) {
        ...
        if (program != 0) {
            ...
            if (linkStatus[0] != GLES20.GL_TRUE) {
                ...
                Log.e(GLES20.glGetProgramInfoLog(program), null);
    ...
    private int loadShader(int shaderType, String source) {
        ...
        if (shader != 0) {
            ...
            if (compiled[0] == 0) {
                ...
                Log.e(GLES20.glGetShaderInfoLog(shader), null);
...
//        "         {\n" +
//        "             vec4 color;\n" +
//        "             color.r =texture2D(tex_y, tc).r + 1.4022 * texture2D(tex_v, tc).r - 0.7011;\n" +
//        "             color.r = (color.r < 0.0) ? 0.0 : ((color.r > 1.0) ? 1.0 : color.r);\n" +
//        "             color.g =texture2D(tex_y, tc).r - 0.3456 * texture2D(tex_u, tc).r - 0.7145 * texture2D(tex_v, tc).r + 0.53005;\n" +
//        "             color.g = (color.g < 0.0) ? 0.0 : ((color.g > 1.0) ? 1.0 : color.g);\n" +
//        "             color.b =texture2D(tex_y, tc).r + 1.771 * texture2D(tex_u, tc).r - 0.8855;\n" +
